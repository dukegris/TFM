...
val NER_PARAMS = Array(
    ...
    Array("ex01_me",  1, 1e-3f, 5e-4f),
    Array("ex10_me", 10, 1e-3f, 5e-4f),
    Array("ex99_me", 99, 1e-3f, 5e-4f)
    Array("ex01_mh",  1, 1e-4f, 5e-3f),
    Array("ex10_mh", 10, 1e-4f, 5e-3f),
    Array("ex99_mh", 99, 1e-4f, 5e-3f)
)
...
//for (bertModel <- 9 to 9) {    
    ...
    //for (nerModel <- 2 to 2) {
        ...
        val ner =             saveModel(
            pipelineModel, 
            "TFM-PUB-" + NER_PARAMS(nerModel)(0).toString + "-" + BERT_MODELS(bertModel)(0).toString)
