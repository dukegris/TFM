...
class TestSparkNLP(sc: SparkContext, spark: SparkSession, modelDirectory: String) {
	...
	val testData = spark.
		...
		val ner_dl = NerDLModel.
				pretrained("ner_dl")

		// load NER model trained by deep learning approach and BERT word embeddings
		val ner_bert = NerDLModel.
				pretrained("ner_dl_bert")

		val french_pos = PerceptronModel.
				load(modelDirectory + "/pos_ud_gsd_fr_2.0.2_2.4_1556531457346/").
	...
	def nlp8: DataFrame = {
		...
		val wordEmbeddings = new WordEmbeddings().
				...
				setEmbeddingsSource("./embeddings.100d.test.txt",100, WordEmbeddingsFormat.TEXT)
		 */
